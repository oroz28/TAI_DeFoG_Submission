...
csv_path = repo_root / "outputs_energy_sweeps/qm9_with_h/energy_sweep_results_with_h.csv"
df = pd.read_csv(csv_path)

# extract mean and std from string columns
df["energy_mean"] = df["energy_mean"].str.extract(r'\(([-0-9\.eE]+),')[0].astype(float)

# for std extract the second value
df["energy_std"] = df["energy_std"].str.extract(r',\s*([-0-9\.eE]+)\)')[0].astype(float)
