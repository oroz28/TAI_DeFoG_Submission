...
from utils import load_fixed_subgraph_masks
###########################################################################
...
def main(cfg: DictConfig):
    ...
    #############################################################################
    X_fixed, E_fixed, node_mask_fixed, edge_mask_fixed = None, None, None, None
    #############################################################################
    ...
    if dataset_config["name"] in [
        ...
        ##############################################################################################
        if hasattr(cfg, "fixed_subgraph_file") and cfg.fixed_subgraph_file not in [None, ""]:
            fixed_subgraph_file = Path(cfg.fixed_subgraph_file)

            total_nodes = torch.argmax(dataset_infos.n_nodes)

            X_fixed, E_fixed, node_mask_fixed, edge_mask_fixed = load_fixed_subgraph_masks(
                fixed_subgraph_file, total_nodes
            )
            visualization_tools = NonMolecularVisualization(
                    dataset_name=cfg.dataset.name,
                    X_fixed=utils.pad_and_expand(X_fixed, (total_nodes,),0, value=-1),
                    E_fixed=utils.pad_and_expand(E_fixed, (total_nodes, total_nodes), 0, value=-1)
            )
    ...
    model_kwargs = {
        ...
        "sampling_metrics": sampling_metrics,
        ####################################################################################
        "visualization_tools": visualization_tools if cfg.visualization else None,
        ####################################################################################
        "extra_features": extra_features,
        "domain_features": domain_features,
        "test_labels": (
        ...
        "X_fixed": X_fixed,
        "E_fixed": E_fixed,
        "node_mask_fixed": node_mask_fixed,
        "edge_mask_fixed": edge_mask_fixed,
        ##########################################################################################
