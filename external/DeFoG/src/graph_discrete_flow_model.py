...
class GraphDiscreteFlowModel(pl.LightningModule):
    def __init__(
        ...
        X_fixed=None,
        E_fixed=None,
        node_mask_fixed=None,
        edge_mask_fixed=None,
        #######################################################################
    ):
        super().__init__()
        ...
        #######################################################################
        # fixed data masking
        self.node_mask_fixed = node_mask_fixed
        if node_mask_fixed is not None:
            self.edge_mask_fixed = edge_mask_fixed
    ...
    def sample(self, is_test, save_samples, save_visualization):
        ...
        while samples_left_to_generate > 0:
            ...
            ##################################################################################
            cur_samples, cur_labels, sampling_time = self.sample_batch(
                graph_id,
                to_generate,
                num_nodes=None,
                save_final=to_save,
        ...
        ##################################################################################
        return samples, labels, sampling_time
        ##################################################################################
    ...
    def sample_batch(
        ...
        if self.conditional:
            if "qm9" in self.cfg.dataset.name:
                ################################################################################################
                condition = torch.tensor([self.cfg.general.condition_values], device=self.device).unsqueeze(0)
        ...
        #######################################################################
        if self.node_mask_fixed is not None:
            print("#######################################################################")
            print("Before padding:")
            print(f"X_fixed: {self.X_fixed.shape}")
            print(f"node_mask_fixed: {self.node_mask_fixed.shape}")
            print(f"E_fixed: {self.E_fixed.shape}")
            print(f"edge_mask_fixed: {self.edge_mask_fixed.shape}")
            print("---------------------------------------------------------------------------")
            print(f"Batch size: {batch_size}")

            # Guardar las máscaras originales si no están guardadas
            if not hasattr(self, "node_mask_fixed_original"):
                self.node_mask_fixed_original = self.node_mask_fixed
                self.X_fixed_original = self.X_fixed
                self.E_fixed_original = self.E_fixed
                self.edge_mask_fixed_original = self.edge_mask_fixed

            # Hacer padding si nunca se hizo o si el batch cambió
            if (not hasattr(self, "fixed_data")) or (self.node_mask_fixed.shape[0] != batch_size):
                node_fixed = utils.pad_and_expand(self.X_fixed_original, (n_max,), batch_size)
                node_mask_fixed = utils.pad_and_expand(self.node_mask_fixed_original, (n_max,), batch_size)
                edge_fixed = utils.pad_and_expand(self.E_fixed_original, (n_max, n_max), batch_size)
                edge_mask_fixed = utils.pad_and_expand(self.edge_mask_fixed_original, (n_max, n_max), batch_size)

                self.fixed_data = utils.PlaceHolder(
                    X=node_fixed,
                    E=edge_fixed,
                    y=None,
                )
                self.fixed_data.to_device(self.device)

                self.node_mask_fixed = node_mask_fixed.to(self.device)
                self.edge_mask_fixed = edge_mask_fixed.to(self.device)
            else:
                # Usar lo ya generado
                node_fixed = self.fixed_data.X
                node_mask_fixed = self.node_mask_fixed
                edge_fixed = self.fixed_data.E
                edge_mask_fixed = self.edge_mask_fixed

            print("\nAfter padding and expanding:")
            print(f"node_fixed: {node_fixed.shape}")
            print(f"node_mask_fixed: {node_mask_fixed.shape}")
            print(f"edge_fixed: {edge_fixed.shape}")
            print(f"edge_mask_fixed: {edge_mask_fixed.shape}")

            z_T.enforce_hard_constraint(
                self.fixed_data, self.node_mask_fixed, self.edge_mask_fixed
            )
        ...
        ############################################################################
        end_time = time.time()
        sampling_time = end_time - ini_time
        ############################################################################
        ...
        ##################################################################################
        return molecule_list, label_list, sampling_time 
        ##################################################################################
    ...
    def sample_p_zs_given_zt(
        ...
        #######################################################################
        if self.node_mask_fixed is not None:
            z_T = utils.PlaceHolder(X=X_s, E=E_s, y=None)
            z_T.enforce_hard_constraint(
                self.fixed_data, self.node_mask_fixed, self.edge_mask_fixed
            )
            X_s, E_s = z_T.X, z_T.E
