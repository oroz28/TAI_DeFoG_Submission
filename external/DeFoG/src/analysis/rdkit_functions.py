...
class BasicMolecularMetrics(object):
    ...
    def cond_sample_metric(self, samples, input_properties):
        ...
        true_properties = []
        ##########################################################################################
        samples = samples[:self.args.general.molecules_energies_to_evaluate]
        input_properties = input_properties[:self.args.general.molecules_energies_to_evaluate]
        ##########################################################################################
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                ###################################################################
                true_properties.append(input_properties[i].reshape(1,-1))
                ###################################################################
            ...
            ##########################################################################################
            if self.args.general.target == "u0":
                u0 = energy
                mols_u0.append(u0)
            ##########################################################################################
        ...
        true_properties = torch.cat(true_properties, dim=0)
        ##########################################################################################
        ...
        ##########################################################################################
        energy_mean, energy_std = None, None
        if self.args.general.target == "u0":
            properties = torch.stack([
                torch.FloatTensor(mols_u0),
            ], dim=1)
            energy_mean = properties.mean()
            energy_std = properties.std()
        ...
        return mae, self.num_valid_molecules / self.num_total, energy_mean, energy_std
        ##########################################################################################
    ...
    def evaluate(self, generated, input_properties=None, test=False):
        ...
        ##########################################################################################
        cond_mae = cond_val = energy_mean = energy_std = -1.0
        if input_properties is not None:
            cond_mae, cond_val, energy_mean, energy_std = self.cond_sample_metric(generated, input_properties)
        ##########################################################################################
        ...
        return (
            ...
            all_smiles,
            ###########################################################################################
            [cond_mae, cond_val, energy_mean, energy_std],
            ###########################################################################################
        )
...
def compute_molecular_metrics(
    ...
    dic = {
        ...
        ##########################################################################################
        "energy_mean": rdkit_metrics[-1][2],
        "energy_std": rdkit_metrics[-1][3],
        ##########################################################################################
    }
    if wandb.run:
        dic = dic
